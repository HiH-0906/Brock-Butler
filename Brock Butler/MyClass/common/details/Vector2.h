...
template<class T>
inline Vector2Template<T> Vector2Template<T>::Abs() const
{
	return Vector2Template{ std::abs(x),std::abs(y) };
}
template <class T>
...
template<class T>
inline Vector2Template<T> Clamp(const Vector2Template<T>& vec, const T& min, const T& max)
{
	return Vector2Template<T>(std::max(std::min(vec.x, max), min), std::max(std::min(vec.y, max), min));
}
